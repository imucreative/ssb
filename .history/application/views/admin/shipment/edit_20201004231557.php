<div class="col-sm-12">
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<table class="table">
				<?php
					echo form_open_multipart('admin/shipment/edit/'.$row['shipment_id']);
				...
				?>
					...
					<tr>
						<td class="col-sm-6" >
							...
							<div class="col-sm-9">
								<input type="text" class="form-control" id="form-field-1" placeholder="" name="code" value="<?php echo $row['code'];?>"  maxlength="20" required />
							</div>
						</td>
						<td class="col-sm-6" >
										...
										<div class="input-group">
											...
											<div class="input-group-btn">
												<div class="btn btn-light-grey btn-file">
													...
													<input type="file" id="form-field-6" name="userfile" class="file-input" value="<?php echo $row['file'];?>" accept=".png, .jpg, .jpeg, .doc, .docx, .pdf"/>
												</div>
												...
												<?php 
													if($row['file']!=""){
														...
														$remove = anchor("admin/shipment/remove_file/".$row['shipment_id'], '<i class="fa fa-times"></i>', ["class"=>"btn btn-danger tooltips", "data-placement"=>'top', "data-original-title"=>'Remove File', "title"=>'Remove File', "onclick"=>"return confirm('Are you sure?')"]);

														echo "
														<a href='".base_url()."admin/shipment/download/".$row['file']."' class='btn btn-info tooltips' data-placement='top' data-original-title='Download File' title='Download File'>
				...
				<tbody>
					<?php
						...
						foreach ($shipment_history as $pi){
							...
							$date = date_create($pi->date_history);

							echo "<tr>
								<td align='center'>".$no."</td>
								<td align='center'>".date_format($date,"d/m/Y H:i:s")."</td>
	...
	<div id="modal_input" class="modal fade" tabindex="-1" data-focus-on="input:first" style="display: none;">
		<?php 
			echo form_open('admin/shipment/post_shipment_history/edit');
			echo form_hidden('shipment_id', $row['shipment_id']);
		?>
