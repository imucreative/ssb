<?php
	class Mod_shipment extends CI_Model{
		...
		//shipment
		function select_all(){
			$query	= "SELECT ship.code, ship.date_of_shipment, ship.to_location, ship.consignee, ship.file, ship.shipment_id
		...
		function delete_shipment_status($id){
			$data = array(
				'status_delete'    => "1"
			);
			$this->db->where('id', $id);
